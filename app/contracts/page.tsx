...
import {
  ...
  type DayActivity,
} from "@/components/contract-activity-summary";
import {
  ContractUpcomingMeetings,
  type UpcomingMeeting,
} from "@/components/contract-upcoming-meetings";
import {
  CONTRACT_MILESTONES,
...
const RESULT_CODES = new Set<string>(
  CONTRACT_RESULT_STATUSES.map((s) => s.code),
);
...
function StatusBadge({
  ...
  return (
    <span className="flex items-center gap-2">
      ...
      {lastMemoDate && (
        <span className="shrink-0 text-[13px] text-neutral-600 tabular-nums">
          {relativeDayKST(lastMemoDate)}
        </span>
      )}
    </span>
  );
...
export default async function ContractsPage({
  ...
  const activeType = CONTRACT_TYPES.some((t) => t.code === typeParam)
    ? typeParam
    : "";
  const statusParam = first(sp.status);
  const activeStatus =
    CONTRACT_STATUSES.some((s) => s.code === statusParam) ||
    statusParam === NONE_STATUS
      ? statusParam
      : "";
  ...
  const activeMonth =
    monthParam || (activeDate ? monthOf(activeDate) : monthOf(today));
  ...
  const statusOf = (r: { memos: { status: string }[] }): string =>
    r.memos[0]?.status ?? NONE_STATUS;
  ...
  const idsForActiveStatus = activeStatus
    ? scopeRows.filter((r) => statusOf(r) === activeStatus).map((r) => r.id)
    : null;
  ...
  const [
    ...
    prisma.contractedVendor.count({
      where: { AND: [scopeWhere, INCOMPLETE_WHERE] },
    }),
    ...
    activeDate
      ? prisma.contractMemo.findMany({
          where: {
            createdAt: {
              gte: kstDayStartUTC(activeDate),
              lt: kstDayStartUTC(addDaysYmd(activeDate, 1)),
            },
          },
          orderBy: [{ createdAt: "desc" }, { id: "desc" }],
          include: {
    ...
    prisma.contractMemo.findMany({
      where: {
        createdAt: {
          gte: kstDayStartUTC(yesterday),
          lt: kstDayStartUTC(addDaysYmd(today, 1)),
        },
      },
    ...
    prisma.contractMemo.findMany({
      where: {
        createdAt: {
          gte: kstDayStartUTC(gridRange.start),
          lt: kstDayStartUTC(gridRange.endExclusive),
        },
      },
      ...
      select: {
        createdAt: true,
        status: true,
        contractedVendor: { select: { managerName: true } },
      },
    }),
    // 달력에 뿌릴 42칸치 신규 등록.
    ...
    activeDate
      ...
      : prisma.contractMemo.findMany({
          where: {
            status: { in: CONTRACT_MILESTONE_CODES },
            contractedVendor: listWhere,
          },
          orderBy: [{ createdAt: "desc" }, { id: "desc" }],
          select: {
            contractedVendorId: true,
            status: true,
            createdAt: true,
            nextContactAt: true,
          },
  ...
  const calendarActivityByStatus =
    countsByCreatedAtAndStatus(monthActivityRows);
  const calendarNewVendors = countsByCreatedAtKST(monthNewRows);

  // ── 다음 연락 예정 ────────────────────────────────────────────────────────
  const livePlans = plannedRows.filter(
    (r) => r.contractedVendor.memos[0]?.id === r.id,
  );
  ...
  const overdueCount = livePlans.filter(
    (r) => r.nextContactAt!.toISOString().slice(0, 10) < today,
  ).length;
  ...
  const dayPlans = activeDate
    ? livePlans.filter(
        (r) => r.nextContactAt!.toISOString().slice(0, 10) === activeDate,
      )
    : [];
  ...
  // 사라져 버리면 그 사실 자체가 안 보인다. 달 집계와 그 날 집계가 같은 규칙을 쓰도록
  // 함수로 뺐다(둘이 다른 규칙을 쓰면 숫자가 서로 안 맞는다).
  const foldManagers = (
    rows: { status: string; managerName: string | null }[],
  ) => {
    ...
    for (const r of rows) {
      ...
      const name = r.managerName?.trim() || "담당자 없음";
    ...
    return (
      [...buckets.entries()]
        ...
        .sort(
          (a, b) =>
            sum(b.counts) - sum(a.counts) || a.name.localeCompare(b.name, "ko"),
        )
        .slice(0, MANAGER_LIMIT)
    );
  };

  const byManager = foldManagers(
    monthActivityRows
      .filter((r) => ymdKST(r.createdAt).slice(0, 7) === activeMonth)
      .map((r) => ({
        status: r.status,
        managerName: r.contractedVendor.managerName,
      })),
  );

  // 날짜를 고른 경우 그 날치도 따로. 달 데이터에서 거르지 않고 그 날 쿼리(dateMemos)에서
  // 뽑는 이유: 날짜를 고른 뒤 달을 넘기면 그 날이 달력 범위 밖으로 나가 버린다.
  const daySummary = activeDate
    ? {
        date: activeDate,
        byStatus: dateMemos.reduce<Record<string, number>>((acc, m) => {
          acc[m.status] = (acc[m.status] ?? 0) + 1;
          return acc;
        }, {}),
        newVendors: dateNewVendors.length,
        plans: dayPlans.length,
        byManager: foldManagers(
          dateMemos.map((m) => ({
            status: m.status,
            managerName: m.contractedVendor.managerName,
          })),
        ),
      }
    : null;
  ...
  const statusScopeTotal = [...statusCounts.values()].reduce(
    (a, b) => a + b,
    0,
  );
  ...
  const statusChips = [
    ...
    })),
    {
      ...
      count: statusCounts.get(NONE_STATUS) ?? 0,
    },
  ];
  ...
  const filterDesc = [
    ...
    activeStatus
      ? activeStatus === NONE_STATUS
        ? "미분류"
        : contractStatusLabel(activeStatus)
      : "",
  ...
  for (const m of dateMemos)
    dateStatusCounts.set(m.status, (dateStatusCounts.get(m.status) ?? 0) + 1);
      ...
      >
        <main className="mx-auto w-full max-w-shell px-4 py-6">
          ...
          <div className="animate-fade-up mb-4 flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-neutral-600">
                오늘·어제 한 일을 먼저 보고, 달력에서 날짜를 누르면 그 날 기록이
                펼쳐집니다.
              </p>
            </div>
          ...
          <div className="grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_21rem]">
                  ...
                  <div className="card-surface animate-fade-up mb-3 px-5 py-4">
                    <div className="flex flex-wrap items-baseline gap-x-2 gap-y-1">
                      <h2 className="text-base font-semibold">
                        {formatDayHeadingKST(activeDate)}
                      </h2>
                      <span className="text-xs text-neutral-600">
                        {relativeDayKST(dateOnlyUTC(activeDate))}
                      </span>
                      <span className="text-sm text-neutral-600 tabular-nums">
                        메모 {nf.format(dateMemos.length)}건 · 신규 등록{" "}
                        {nf.format(dateNewVendors.length)}곳
                        {dayPlans.length > 0 &&
                          ` · 예정 ${nf.format(dayPlans.length)}건`}
                      </span>
                    </div>
                    {dateStatusCounts.size > 0 && (
                      <div className="mt-2.5 flex flex-wrap gap-1.5">
                        {CONTRACT_STATUSES.filter((s) =>
                          dateStatusCounts.get(s.code),
                        ).map((s) => (
                          <span
                            key={s.code}
                  ...
                  {dayPlans.length > 0 && (
                    <div className="card-surface animate-fade-up mb-3 overflow-hidden">
                      <h3 className="border-b border-black/[0.06] px-5 py-2.5 text-sm font-semibold">
                        이 날 예정{" "}
                        <span className="ml-1 font-normal text-neutral-600 tabular-nums">
                          {dayPlans.length}건
                        </span>
                        <span className="ml-2 text-xs font-normal text-neutral-600">
                          (미리 잡아둔 일정 — 위의 &lsquo;한 일&rsquo;과
                          별개입니다)
                        </span>
                      </h3>
                            ...
                            >
                              <span className="inline-flex items-center gap-1.5 rounded-full bg-neutral-100 px-2 py-0.5 text-xs font-medium text-neutral-700">
                                <span
                                  ...
                                  className="size-1.5 rounded-full"
                                  style={{
                                    backgroundColor: contractStatusDot(
                                      r.status,
                                    ),
                                  }}
                                />
                          ...
                          return (
                            <li key={m.id} className="px-5 py-4">
                              <div className="flex flex-wrap items-center justify-between gap-2">
                                ...
                                <span
                                  className="text-[13px] text-neutral-600 tabular-nums"
                    ...
                    <div className="animate-fade-up rounded-2xl border border-dashed border-black/15 bg-white py-16 text-center">
                      ...
                      {!filterDesc && (
                        <p className="mt-1.5 text-sm text-neutral-600">
                          위의 &lsquo;업체 빠르게 추가&rsquo;를 펼쳐 업체명과
                          담당자만으로 등록할 수 있습니다.
                        </p>
                      )}
                    </div>
                    ...
                    <div className="card-surface animate-fade-up overflow-hidden">
                            ...
                            return (
                              <ContractRow key={v.id} id={v.id}>
                                <TableCell>
                                  ...
                                  >
                                    <span
                                      ...
                                      style={{
                                        backgroundColor: contractTypeDot(
                                          v.contractType,
                                        ),
                                      }}
                                ...
                                <TableCell className="text-right text-[13px] text-neutral-600 tabular-nums">
                                  <span title={formatDateTimeKST(v.createdAt)}>
                                    {relativeDayKST(v.createdAt)}
                                  </span>
                                  {incomplete && (
                                    <span className="sr-only">
                                      {" "}
                      ...
                      {vendors.length >= MAX_ROWS && (
                        <p
                          ...
                          style={{ color: "var(--data-warning-ink)" }}
                        >
                          ▲ 최근 {nf.format(MAX_ROWS)}곳만 표시하고 있습니다
                          (조건에 맞는 업체는 {nf.format(matchingTotal)}곳).
                          검색어나 필터로 범위를 좁혀주세요.
                        </p>
                      )}
                    </div>
            ...
            <aside
              className="animate-fade-up space-y-4 lg:sticky lg:top-[4.5rem]"
            ...
            >
              ...
              <section className="card-surface px-4 py-3">
                ...
                <dl className="divide-y divide-black/[0.06]">
                  ...
                  <RailStat
                    label="구두 계약만"
                    value={typeCounts.get("verbal") ?? 0}
                    dot="var(--data-contract-verbal)"
                  />
                  <RailStat
                    label="정보 미비"
                ...
                {showMeter && (
                  <div className="mt-2.5 border-t border-black/[0.06] pt-2.5">
                    ...
                    <p className="mt-1.5 text-xs text-neutral-600 tabular-nums">
                      전화번호·주소까지 받은 곳 {nf.format(completeCount)} /{" "}
                      {nf.format(scopeTotal)}
                    </p>
                  </div>
                )}
              </section>

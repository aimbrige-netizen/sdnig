...
원본은 그대로 보관하되 **화면에는 필요한 크기만큼만** 내려받습니다. 목록의 40px 타일에
3~5MB 원본을 그대로 받으면 사진 52장짜리 업체 화면이 25MB 를 받게 되기 때문입니다.

`next.config.ts` 의 `remotePatterns` 가 Supabase 호스트(환경변수에서 읽음)와 유튜브 썸네일
호스트를 허용하고, 화면의 모든 사진은 `next/image` 로 렌더링합니다. 실측하면 874KB 원본이
64px 타일에서 **350바이트**로 줄어듭니다. 화면 밖 사진은 스크롤해야 받습니다(lazy).

Vercel Hobby 는 이미지 변환 월 5,000회 · 캐시 읽기 30만 · 쓰기 10만이 무료이고,
**한도를 넘겨도 청구되지 않고 최적화만 멈춥니다**(이미 변환된 것은 계속 동작).

사진을 누르면 크게 보는 화면이 뜨고, 거기서 [원본 다운로드] 로 올릴 때 그대로인 원본을
저장하거나 [새 탭에서 열기] 로 볼 수 있습니다 (`components/vendor-form/photo-thumb.tsx`).
[사진관리] 탭 맨 위의 [원본 전체 다운로드] 는 그 업체의 사진·영상을 한 번에 받습니다.

### 원본 다운로드

저장된 파일명은 UUID 라 그대로 받으면 어느 업체 사진인지 알 수 없습니다. 받을 때
`업체명-갤러리 2 홀 전경.jpg` 처럼 이름을 붙입니다 (`lib/vendor-downloads.ts`).

파일명을 붙이는 방법이 두 겹입니다:

1. **blob 방식(주 경로)** — 파일을 먼저 받아 `blob:` 주소로 바꿔 저장합니다. blob 은 같은
   도메인 취급이라 `<a download>` 이 그대로 먹고, 한글 이름이 확실히 붙습니다.
   Supabase 공개 파일이 `Access-Control-Allow-Origin: *` 라 가능합니다.
2. **`?download=<파일명>` (폴백)** — 받아오지 못하면 서버가 `Content-Disposition` 으로
   이름을 붙여줍니다. Supabase Storage 가 지원하는 규약이고, 로컬 서빙 라우트도 같은
   규약을 구현했습니다(`filename=` 과 `filename*=UTF-8''` 둘 다 보냄).

영상 링크(유튜브 등)는 우리 파일이 아니므로 다운로드 목록에서 제외합니다.
